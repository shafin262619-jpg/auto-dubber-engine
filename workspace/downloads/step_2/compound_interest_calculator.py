...
class CompoundInterestResult(TypedDict):
    """Structured result of a compound interest calculation."""
    future_value: float
...
def calculate_compound_interest(
    ...
    Returns a CompoundInterestResult containing the future value, total
    ...
    # Compute the growth factor once and reuse it below — this is the
    # most expensive operation in the function (fractional exponentiation),
    # and the original version computed it twice.
    growth_factor = (1 + periodic_rate) ** (n * t)

    # Future value of the initial principal
    principal_growth = principal * growth_factor

    # Future value of the periodic contributions (ordinary annuity)
    if r == 0:
        contribution_growth = contribution * n * t
    else:
        contribution_growth = contribution * ((growth_factor - 1) / periodic_rate)
